	...
	methods: {
		...
		tapQuery(item) {
			...
			if (this.showMaskType === 'characteristics') {
				...
				let servicesData = this.servicesData;
				let characteristicsData = this.characteristicsData;
				uni.navigateTo({
		...
		createBLEConnection() {
			...
			uni.createBLEConnection({
				...
				success: res => {
					...
					uni.setStorageSync('device',this.equipment);
		...
		closeBLEConnection() {
			...
			uni.closeBLEConnection({
				...
				success: res => {
					...
					this.$set(this.disabled, 11, true);
					this.equipment = [];
					this.servicesData = [];
			...
			uni.getBLEDeviceCharacteristics({
				...
				success: res => {
					...
					for (var i = this.list.length - 1 ; i >= 0 ; i--) {
						if(this.list[i].properties.write == 1){
							var bar1 = this.list[i];
							console.log(bar1);
							this.tapQuery(bar1);
							console.log('页面马上跳转喽')
						}
